...
const carouselItems: NodeListOf<Element> = document.querySelectorAll('.carousel-selector-images li');
const flavorCardItems: NodeListOf<Element> = document.querySelectorAll('.flavor-card');

// Add click event listeners to carousel items
for (let i = 0; i < carouselItems.length; i++) {
    carouselItems[i].addEventListener('click', function (this: HTMLElement) {

        const clickedItem = this as HTMLElement;

        turnFlavorInvisible(carouselItems);
        turnFlavorColorVisible(clickedItem);
        toggleFlavorCard(clickedItem);
    });
}

// Turn off all the indicators that the flavor is selected
function turnFlavorInvisible(items: string | any[] | NodeListOf<Element>) {
...
function turnFlavorColorVisible(item: HTMLElement ) {
...
function toggleFlavorCard(selectedItem: HTMLElement) {
